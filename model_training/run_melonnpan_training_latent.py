...
for dp in ["default"]:
    for model_output in models.keys():
        for model_input in models[model_output]:
            if "+" not in model_input: continue
            
            # Skip multi-omics
            if args["single_omics"] and "+" in model_input: continue
            
            input_data = "%s%s/seed_%s/melonnpan/train/to_%s/from_%s_input_latent.txt" % (config.input_data_root,
                                                                                          dp,
                                                                                          seed,
                                                                                          model_output,
                                                                                          model_input)
            # MelonnPan applies an arcsin transformation to the output by default
            output_folder = "%s%s/seed_%s/melonnpan/output_%s_to_%s_latent/" % (config.output_data_root,
                                                                                dp,
                                                                                seed,
                                                                                model_input,
                                                                                model_output)
            ...
            training_command += " --no.transform.metag"
            ...
            predictions_command += " --no.transform.metag"
